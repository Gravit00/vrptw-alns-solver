"""
VRPTW - ALNS + 2-opt Solver (Adaptive Weights)
Adaptive Large Neighbourhood Search with adaptive operator weight selection
and intra-route 2-opt local search for the Vehicle Routing Problem with Time
Windows.

Pipeline:
1. Parse Solomon benchmark (C101)
2. Nearest Neighbour initial solution
3. ALNS main loop (adaptive Random/Worst Removal + Greedy Insertion + 2-opt + SA acceptance)
...
# ── 9. ALNS Main Loop (Adaptive Weights + 2-opt + SA) ────────────────────────
def alns(
    initial_solution,
    n_iterations=1000,
    n_remove=5,
    ...
    Each iteration:
        ...
        3. Intra-route 2-opt
        4. SA acceptance
        5. Update operator weight based on outcome
...
def main():
    ...
    plot_routes(nodes, alns_sol.routes, title='ALNS (Adaptive) — c101')
...
def plot_comparison(nn_sol, alns_sol, history, weight_history, destroy_names):
    ...
    plt.suptitle('VRPTW c101: Nearest Neighbour vs ALNS (Adaptive+2OPT)', fontsize=13, fontweight='bold')
    plt.tight_layout()
    plt.savefig('comparison_adaptive_2opt-c101.png', dpi=150, bbox_inches='tight')
    plt.show()

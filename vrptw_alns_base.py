"""
VRPTW - ALNS Solver
Adaptive Large Neighbourhood Search for the Vehicle Routing Problem with Time Windows.

Pipeline:
1. Parse Solomon benchmark (C101)
2. Nearest Neighbour initial solution
3. ALNS main loop (Random/Worst Removal + Greedy Insertion + SA acceptance)
...
def worst_removal(solution, n_remove):
    ...
    Saving for customer c at position (prev, c, next):
        saving = dist(prev, c) + dist(c, next) - dist(prev, next)
    """
    sol = solution.copy()
    removed = []
...
def greedy_insertion(solution, removed):
    ...
    Customers sorted by due_date ascending (tighter deadlines first).
    If no feasible position exists, open a new single-customer route.
    """
    sol = solution.copy()
    dist = sol.dist_matrix
    nodes = sol.nodes
...
def alns(
    ...
    """
    Adaptive Large Neighbourhood Search for VRPTW.
    ...
    for iteration in range(1, n_iterations + 1):
        ...
        # Repair
        candidate = greedy_insertion(destroyed, removed)
...
def main():
    ...
    plot_routes(nodes, alns_sol.routes, title='ALNS — rc101')
...
def plot_comparison(nn_sol, alns_sol, history):
    ...
    plt.suptitle('VRPTW c101: Nearest Neighbour vs ALNS(no adaptive, no 2OPT)', fontsize=13, fontweight='bold')
    plt.tight_layout()
    plt.savefig('comparison-noadaptive-no2opt-c101.png', dpi=150, bbox_inches='tight')
    plt.show()
